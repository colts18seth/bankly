...
describe("POST /auth/login", function () {
    ...
    // TEST BUG #1 
    test("should not return token if username/password is incorrect", async function () {
...
describe("GET /users", function () {
    ...
    // TEST BUG #2 
    test("should deny access if token isn't verified", async function () {
        const response = await request(app).get("/users")
...
describe("GET /users/[username]", function () {
    ...
    // TEST BUG #3
    test("should throw error when username is not found", async function () {
        const response = await request(app)
            .get("/users/test")
            .send({ _token: tokens.u1 });
...
describe("PATCH /users/[username]", function () {
    ...
    // TEST BUG #4
    test("should allow access if admin/right user", async function () {
        const response = await request(app)
            .patch("/users/u2")
            .send({
                _token: tokens.u2,
                first_name: "u2 Updated"
            });
        expect(response.statusCode).toBe(200);
